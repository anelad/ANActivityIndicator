//
//  ANActivityIndicatorAnimationCubeTransition.swift
...
class ANActivityIndicatorAnimationCubeTransition: ANActivityIndicatorAnimation {
    required init() {
    ...
    func setUpAnimation(in layer: CALayer, size: CGSize, color: UIColor) {
      ...
      let timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.easeInEaseOut)
        ...
        // Draw squares
        for i in 0 ..< 2 {
            let square = ANActivityIndicatorShape.rectangle.layerWith(size: CGSize(width: squareSize, height: squareSize), color: color)
